...
// ---- Token helpers (đa định dạng) ----
function getTokens() {
  // 0) key mà store đang dùng
  try {
...
function setTokens(next) {
  ...
  try {
    if (next) {
      ...
      localStorage.setItem('auth', JSON.stringify({ ...curr, accessToken: next.accessToken, refreshToken: next.refreshToken }));
    } else {
...
/**
 * request() có auto Bearer + auto refresh 1 lần
 */
async function request(path, { method = 'GET', headers = {}, body, params, _retried } = {}) {
  const isAbs = /^https?:\/\//i.test(path);
  const rel = isAbs ? path : stripApiPrefix(path);
  let url = isAbs ? rel : `${BASE}${rel}`;

  if ((method === 'GET' || method === 'DELETE') && params) url = withParams(url, params);

  const h = { Accept: 'application/json', ...headers };
  const isFormData = typeof FormData !== 'undefined' && body instanceof FormData;
  if (!isFormData && body != null) h['Content-Type'] = h['Content-Type'] || 'application/json';
  ...
  if (res.status === 401 && !_retried && tokens?.refreshToken) {
    try {
      const refreshed = await rawPost('/auth/refresh', { refreshToken: tokens.refreshToken });
      const next = { accessToken: refreshed.accessToken, refreshToken: refreshed.refreshToken || tokens.refreshToken };
      setTokens(next);
